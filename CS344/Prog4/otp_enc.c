...
// otp_enc acts as the client. It will accept input in the form:
...
int main(int argc, char *argv[]) {
	...
	/******************************************************
 	 * otp_enc_d expects to receive "otp_enc" as its
 	 * first argument, anything else and it will not respond
 	 * to another client.
	 * The daemon responds with stop or go signal
 	 *****************************************************/
	int otpBytesSent = send(socketFD, "otp_enc", 7, 0);
	if (otpBytesSent < 0) error("CLIENT: ERROR reading otp_enc to server");
	...
	if (textFile == NULL) error("CLIENT: ERROR opening the plaintext file");
	else {
	 ...
	 * send that to otp_enc_d.
	 ****************************************************/

	char textBuffer[4096];
	memset(buffer, '\0', sizeof(textBuffer));
	int textBytesRead = fread(textBuffer, sizeof(char), sizeof(textBuffer), textFile);
	if (textBytesRead < 0) error("CLIENT: ERROR reading from plaintext file");

	// make sure that the characters in the plaintext file are legal!
	int i;
	for (i = 0; i < textSize - 1; i++) {
		if (islower(textBuffer[i])) {
			if (textBuffer[i] != ' ' && textBuffer[i] != '\n' && textBuffer[i] != '\0' && textBuffer[i] != 22) {
				// printf("%c is a bad character.\n", textBuffer[i]);
				error("CLIENT: ERROR plaintext contains bad characters\n");
			}
		}
	}
	...
	// all of the data may not be written in one call.
		...
		if (textBytesSent < 0) error("CLIENT: ERROR writing plaintext to socket");
	...
	// FIXME while loop probably should go here
	...
	/****************************************************
 	 * Finally, receive the encrypted text from otp_enc_d
 	 ***************************************************/
