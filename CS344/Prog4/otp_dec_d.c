...
int main(int argc, char *argv[]) {
	...
	while(1) {
		...
		if (childPid == 0) {
			 ...
			 * from otp_dec, this program expects that
			 * the first arg sent is "otp_dec".
			 ****************************************/
			char otpBuffer[256];
			...
			if (strcmp(otpBuffer, "otp_dec") != 0) {
				send(establishedConnectionFD, "STOP", 4, 0);
				error("SERVER: Please connect to otp_dec_d only with otp_dec!");
			}
			else
			...
			for (i = 0; i < strlen(textBuffer); i++) {
				if (textBuffer[i] == ' ') {
					...
					if (num < 0)
						num = num + 26;
					decryptBuffer[i] = num + 65;
				}
			}

			// finally, return the encrypted text!
			charsRead = send(establishedConnectionFD, decryptBuffer, strlen(decryptBuffer), 0);
