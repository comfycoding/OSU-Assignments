...
bool HarryPotter::attack(Creature *defender, string def)
{
	...
	if (defender->getSP() <= 0) {
		...
		wins++;
		heal();
		Hogwarts = true;		// reset the Harry Potter ability.
		if (strengthPoints > 20)
			strengthPoints = 20; // and he at least won't go over 20 SP.
		cout << "Harry Potter now has " << wins << " wins, and his SP is now at " << strengthPoints << "." << endl;
		return false;
	}
	else {
